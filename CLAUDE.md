...
1. **`src/assistant/`** — defines Everly herself. `prompt.ts` is the entire conversational script (playbook-derived, heavily hand-tuned through many live-call iterations — treat edits here as high-risk, keep them additive) with `{{variable}}` placeholders. `tools.ts` defines 9 custom function-calling tools (see below) plus Vapi's native `endCall`. `create.ts` pushes both to Vapi via `POST`/`PATCH /assistant`.
...
9 custom function-calling tools, all routed through `POST /vapi/tool-calls` → `webhookHandlers.ts`'s `dispatchTool`, plus Vapi's native `endCall`:
